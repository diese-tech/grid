...
export function DossierPanel({
  ...
  return (
    <aside
      className={`relative flex min-h-[26rem] flex-col border border-cyan-100/24 bg-slate-950/34 p-4 shadow-2xl shadow-cyan-950/20 backdrop-blur-[3px] transition duration-500 ${
        open ? "translate-y-0 opacity-100" : "translate-y-4 opacity-75"
      }`}
      aria-label={`${node.label} dossier`}
    >
      <div className="absolute -top-7 left-6 h-7 w-20 border-x border-t border-cyan-100/20 bg-slate-950/34 backdrop-blur-[3px]" />
      ...
      <div className="mt-4 min-h-0 flex-1 overflow-y-auto pr-2">
        ...
        <dl className="mt-5 grid grid-cols-2 gap-3 text-sm">
          <div className="border border-cyan-100/10 bg-cyan-100/8 p-3">
        ...
        <section className="mt-5">
          ...
          <ul className="mt-2 grid gap-2">
            {relatedNodes.map((relatedNode) => (
              <li
                key={relatedNode.id}
                className="border border-cyan-100/10 bg-black/18 px-3 py-2 text-sm text-slate-300"
              >
                {relatedNode.label}{" "}
